        ...
        <div class="row" style="height: 100%;">
            <div id="cell-sidebar" class="cell bg-lightDark size-x300" style="overflow-y: scroll; height: 100%;">
                <ul class="sidebar2 dark">
                    ...
                    <li class="divider"></li><li class="divider"></li>
                    <li class="active"><a href="./manage_faculties.php"><span class="mif-users mif-ani-shuttle icon"></span><span class="title">Manage Faculty Information</span></a></li>
            ...
            <div id="cell-content" class="cell auto-size padding20 pane-scroll">
                <h1 class="text-light">Manage Faculty Information</h1>
                <hr><br>
                <div class="align-right">
                    <a class="button primary beautify beautify-mif" data-button="add-faculty-button" data-role="hint" data-hint="Add Faculty" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-plus"></span>&nbsp;&nbsp;Add</a>
                </div>
                <table id="faculties-table" class="table striped border bordered">
                    ...
                    <tbody>
                        <?php
                            ...
                            $connection->query("SELECT * FROM accounts INNER JOIN faculties ON accounts.Account_ID=faculties.Faculty_ID");

                            while($row = $connection->fetch_assoc()) {
                                if(strlen($row['Faculty_Middle_Name']) > 1) {
                                    $name = $row['Faculty_Last_Name'] . ', ' . $row['Faculty_First_Name'] . ' ' . substr($row['Faculty_Middle_Name'], 0, 1) . '.';
                                } else {
                                    $name = $row['Faculty_Last_Name'] . ', ' . $row['Faculty_First_Name'];
                                }

                                echo '<tr>';
                                echo '<td class="align-center">' . $row['Faculty_ID'] . '</td>';
                                echo '<td>' . $name . '</td>';
                                echo '<td>' . $row['On_Hand'] . '</td>';
                                echo '<td>' . $row['Account_Type'] . '</td>';
                                echo '<td class="align-center"><a class="button success mini-button beautify" data-button="edit-faculty-button" data-var-id="' . $row['Faculty_ID'] . '" data-role="hint" data-hint="Edit Faculty" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-pencil"></span></a><a class="button danger mini-button beautify" data-button="delete-faculty-button" data-var-id="' . $row['Faculty_ID'] . '" data-role="hint" data-hint="Delete Faculty" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-cross"></span></a></td>';
                                echo '</tr>';
                            }
