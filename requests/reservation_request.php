<?php
    ...
    if(isset($username)) {
        if($action == 'Add') {
            ...
            $connection->query("SELECT * FROM reservations WHERE Material_ID='$id' AND Borrowers_ID='$username' AND Status='active'");

            if($connection->num_rows() == 0) {
                $connection->query("INSERT INTO reservations (Material_ID, Borrowers_ID, Date_Reserved) VALUES ('$id', '$username', '$dateReserved')");

                if($connection->affected_rows() == 1) {
                    echo json_encode(array('status' => 'Success', 'message' => 'Reservation has been added.'));
                } else {
                    echo json_encode(array('status' => 'Failed', 'message' => 'Failed to reserve material.'));
                }
            } else {
                echo json_encode(array('status' => 'Failed', 'message' => 'A copy of this material has already been reserved.'));
            }
